...
* Deny policies now always take precedence over allow policies. Previously, a CIDR-based
  ...
  to deny the same traffic. Now, the CIDR-based deny policy will drop traffic when there is
  an allow policy for the same traffic. Verify that all of your CIDR-based deny and allow policies work
